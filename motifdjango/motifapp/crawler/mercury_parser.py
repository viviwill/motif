...
class Crawler(object):
    ...
    def crawl(self):
        ...
        self.raw_content = article['content']
    ...
    def parse_content(self, content):
        ...
        for row in raw_content:
            ...
            row = re.sub(r'<p(.*?)>', "<p class=\"article-body-content\">", row.encode("utf-8"))
            row = re.sub(r'<a(.*?)href', "<a class=\"article-body-href\" href", row)
    ...
    def get_author(self):
        soup = bs4.BeautifulSoup(requests.get(self.web_url).text.encode('utf-8'), "html.parser")
